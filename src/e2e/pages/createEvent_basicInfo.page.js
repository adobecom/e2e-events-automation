...
class BasicInfo extends EventsBasePage {
    ...
    constructor() {
        ...
        this.locators = {
            ...
            cloudTypeDropdown: '#bu-select-input',
            seriesTypeDropdown: '#series-select-input',
            cloudTypeOption: (value) => `sp-picker#bu-select-input sp-menu-item:text("${value}")`,
            seriesTypeOption: (value) => `sp-picker#series-select-input sp-menu-item:text("${value}")`,
            ...
            eventDescriptionRTE: '.event-info-component div[contenteditable="true"].tiptap.ProseMirrors',
            eventDate: '[name="event-date"]',
            enabledDatesClass: '.calendar-day:not(.disabled):not(.empty)',
            iconCalendar: '.icon.icon-calendar-add',
            startTime: '#time-picker-start-time',
            endTime: '#time-picker-end-time',
            startTimePeriod: '#ampm-picker-start-time',
            endTimePeriod: '#ampm-picker-end-time',
            timezone: '#time-zone-select-input',
            startTimeOption: (time) => `sp-picker#time-picker-start-time sp-menu-item:text("${getTimeWithoutPeriod(time)}")`,
            startTimePeriodOption: (time) => `sp-picker#ampm-picker-start-time sp-menu-item:text("${getPeriodFromTime(time)}")`,
            endTimeOption: (time) => `sp-picker#time-picker-end-time sp-menu-item:text("${getTimeWithoutPeriod(time)}")`,
            endTimePeriodOption: (time) => `sp-picker#ampm-picker-end-time sp-menu-item:text("${getPeriodFromTime(time)}")`,
            timezoneOption: (timezone) => `sp-picker#time-zone-select-input sp-menu-item:text("${timezone}")`,
            venueName: 'sp-textfield[placeholder="Venue name"] input[placeholder="Venue name"]',
            firstVenueNameOption: '.pac-item:first-child',
            venueInfoWillAppearPostEventCheckbox: 'sp-checkbox#checkbox-venue-info-visible',
            nextStepButtonEnabled: 'a.next-button:not(.disabled)',
            eventTopicsCheckbox: (name) => `#form-step-basic-info sp-action-button[name="${name}"]`,
            ...
            agendaPostEventCheckbox: 'sp-checkbox#checkbox-agenda-info input[name="checkbox-agenda-info-name"]',
            ...
            languageDropdown: '#language-picker',
            languageOption: (language) => `sp-menu-item[value='${language}'][role='option']`,
            venueAdditionalInfoImageDropzone: 'image-dropzone[id="add-imagefile-dimensions-1920px-wide."] input.img-file-input',
            venueAdditionalInfoRTE: 'input#venue-additional-info-rte-output',
            venueAdditionalInfoCheckbox: 'sp-checkbox#checkbox-venue-additional-info-visible',
